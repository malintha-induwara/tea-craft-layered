...
public class PackagingDetailsModel {
    ...
    public boolean addPackagingDetails(PackagingDetailsDto dto) throws SQLException {
        ...
        pstm.setDate(2,java.sql.Date.valueOf(dto.getDate()));
        pstm.setString(3,dto.getPackId());
        pstm.setInt(4,dto.getCount());
    ...
    public double getTotalDecreasedAmount(String teaTypeId) throws SQLException {
        ...
        String sql = "SELECT SUM(amount) FROM packaging_details WHERE packId IN (SELECT packId FROM packaging WHERE typeId=?) AND isConfirmed=0";
    ...
    public List<PackagingCountAmountDto> getTotalCountAmount(LocalDate parse) throws SQLException {
        ...
        List<PackagingCountAmountDto> dtoList = new ArrayList<>();

        ResultSet resultSet = pstm.executeQuery();


        while (resultSet.next()){
            PackagingCountAmountDto dto = new PackagingCountAmountDto(
                    resultSet.getString(1),
                    resultSet.getInt(2),
